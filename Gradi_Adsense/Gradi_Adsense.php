...
class Gradi_Adsense extends Module implements WidgetInterface
{
	...
	public function __construct()
	{
		...
		$this->version = '1.0.0';
		$this->author = 'Sergio Aguas';
		$this->need_instance = 0;

        $this->bootstrap = true;
        parent::__construct();

        $this->displayName = $this->trans('Gradi Adsense', array(), 'Modules.Banner.Admin');
        $this->description = $this->trans('Creacion para gradiweb', array(), 'Modules.Banner.Admin');

        $this->ps_versions_compliancy = array('min' => '1.7.1.0', 'max' => _PS_VERSION_);

        $this->templateFile = 'module:Gradi_Adsense/Gradi_Adsense.tpl';
    }
    ...
    protected function installFixtures()
    {
        ...
        foreach ($languages as $lang) {
            $this->installFixture((int)$lang['id_lang'], 'santafe.png');
        }
    ...
    {
        if (!$this->isCached($this->templateFile, $this->getCacheId('Gradi_Adsense'))) {
            $this->smarty->assign($this->getWidgetVariables($hookName, $params));
        }

        return $this->fetch($this->templateFile, $this->getCacheId('Gradi_Adsense'));
    }
